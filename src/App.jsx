...
async function getEquityScalper(label) {
  ...
  const sys = "You are a professional equity risk assistant for traders. Respond ONLY with valid JSON. No markdown. Start with { and end with }." +
    ...
    " EQUITY SCALPER schema: " EQUITY SCALPER schema: {ticker:string,risk_level:GREEN|YELLOW|RED,risk_reason:string,scalper_note:string,earnings_proximity:SAFE|NEAR|IMMINENT,breaking:[{headline:string,direction:BULLISH|BEARISH|NEUTRAL,age:string}],imminent:[{event:string,due_in:string,expected_impact:string}]}";
