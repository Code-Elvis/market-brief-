...
return (
<div style={{ background: “#050709”, color: “#e0e0e0”, fontFamily: “system-ui, sans-serif” }}>
<LandingPage onSignIn={() => setView(“sign-in”)} onSignUp={() => setView(“sign-up”)} />
<div id=“auth” style={{ background: “#0a0c0f”, borderTop: “1px solid rgba(255,255,255,.06)”, display: “flex”, flexDirection: “column”, alignItems: “center”, padding: “60px 20px 80px” }}>
<div style={{ marginBottom: 28, textAlign: “center” }}>
<div style={{ fontSize: 20, fontWeight: 800, color: “#fff”, letterSpacing: “-0.5px” }}>MARKET<span style={{ color: “#00d4ff” }}>DEBRIEFS</span></div>
<div style={{ fontSize: 12, color: “#444”, marginTop: 6 }}>{view === “sign-in” ? “Sign in to your account” : “Create your free account”}</div>
</div>
...
const style = `@import url('https://fonts.googleapis.com/css2?family=Syne:wght@700;800;900&family=DM+Sans:wght@300;400;500&display=swap'); html { scroll-behavior: smooth; } * { box-sizing: border-box; margin: 0; padding: 0; } body { background: #050709; } .land-btn-primary { display: inline-block; padding: 16px 36px; border-radius: 6px; background: #00d4ff; color: #000; font-size: 15px; font-weight: 700; text-decoration: none; letter-spacing: 0.5px; font-family: 'Syne', sans-serif; transition: opacity .15s, transform .15s; } .land-btn-primary:hover { opacity: .88; transform: translateY(-1px); } .land-btn-ghost { display: inline-block; padding: 16px 36px; border-radius: 6px; border: 1px solid rgba(255,255,255,.12); color: #777; font-size: 15px; font-weight: 500; text-decoration: none; letter-spacing: 0.5px; font-family: 'DM Sans', sans-serif; transition: border-color .15s, color .15s; } .land-btn-ghost:hover { border-color: rgba(255,255,255,.3); color: #bbb; } .feature-card { background: rgba(255,255,255,.025); border: 1px solid rgba(255,255,255,.07); border-radius: 12px; padding: 28px; transition: border-color .2s, background .2s; } .feature-card:hover { border-color: rgba(0,212,255,.2); background: rgba(0,212,255,.04); } .ticker-row { display: flex; gap: 0; overflow: hidden; white-space: nowrap; border-top: 1px solid rgba(255,255,255,.05); border-bottom: 1px solid rgba(255,255,255,.05); } @keyframes ticker { from { transform: translateX(0) } to { transform: translateX(-50%) } } .ticker-inner { display: flex; gap: 0; animation: ticker 22s linear infinite; } .ticker-item { padding: 10px 28px; font-family: monospace; font-size: 11px; color: #2a2a2a; border-right: 1px solid rgba(255,255,255,.04); } @keyframes fadeup { from { opacity: 0; transform: translateY(18px) } to { opacity: 1; transform: translateY(0) } } .hero-anim { animation: fadeup .7s ease both; } .hero-anim-1 { animation-delay: .1s; } .hero-anim-2 { animation-delay: .22s; } .hero-anim-3 { animation-delay: .36s; } .hero-anim-4 { animation-delay: .5s; } @keyframes pulse-dot { 0%,100% { opacity: 1 } 50% { opacity: .3 } } @media (max-width: 640px) { .land-grid-3 { grid-template-columns: 1fr !important; } .land-grid-2 { grid-template-columns: 1fr !important; } .land-compare { grid-template-columns: 1fr !important; } .land-compare .compare-left { border-radius: 12px 12px 0 0 !important; } .land-compare .compare-right { border-radius: 0 0 12px 12px !important; } .land-hero-h1 { font-size: 36px !important; letter-spacing: -1px !important; } .land-pricing-grid { grid-template-columns: 1fr !important; } .land-nav { padding: 14px 18px !important; } .land-section { padding-left: 18px !important; padding-right: 18px !important; } .land-footer { flex-direction: column !important; text-align: center; padding: 20px 18px !important; } .land-cta-btn { font-size: 14px !important; padding: 16px 28px !important; } }`;
...
```
  ...
  <nav className="land-nav" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "18px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid rgba(255,255,255,.04)", background: "rgba(5,7,9,.85)", backdropFilter: "blur(12px)" }}>
    ...
    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
      <button onClick={() => { if(onSignIn) onSignIn(); document.getElementById("auth")?.scrollIntoView({behavior:"smooth"}); }} style={{ fontSize: 13, color: "#555", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", padding: "8px 16px" }}>Sign in</button>
      <button onClick={() => { if(onSignUp) onSignUp(); document.getElementById("auth")?.scrollIntoView({behavior:"smooth"}); }} className="land-btn-primary" style={{ padding: "9px 20px", fontSize: 13, border: "none", cursor: "pointer" }}>Start free</button>
    </div>
  </nav>
  ...
  <section style={{ paddingTop: 140, paddingBottom: 80, paddingLeft: 24, paddingRight: 24, textAlign: "center", maxWidth: 780, margin: "0 auto" }}>
    ...
    <div className="hero-anim hero-anim-4" style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 20 }}>
      <button onClick={() => { if(onSignUp) onSignUp(); document.getElementById("auth")?.scrollIntoView({behavior:"smooth"}); }} className="land-btn-primary" style={{ border: "none", cursor: "pointer" }}>Start free — 3 briefs/day</button>
      <button onClick={() => { if(onSignIn) onSignIn(); document.getElementById("auth")?.scrollIntoView({behavior:"smooth"}); }} className="land-btn-ghost" style={{ border: "1px solid rgba(255,255,255,.12)", cursor: "pointer", background: "none" }}>Sign in</button>
    </div>
  ...
  <section className="land-section" style={{ maxWidth: 680, margin: "0 auto 100px", padding: "0 24px", textAlign: "center" }}>
    ...
    <div className="land-pricing-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, textAlign: "left" }}>
      ...
      <div style={{ border: "1px solid rgba(255,255,255,.08)", borderRadius: 14, padding: "30px 26px" }}>
        ...
        <button onClick={() => { if(onSignUp) onSignUp(); document.getElementById("auth")?.scrollIntoView({behavior:"smooth"}); }} style={{ display: "block", width: "100%", textAlign: "center", marginTop: 24, padding: "12px", borderRadius: 8, border: "1px solid rgba(255,255,255,.08)", color: "#555", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", background: "none" }}>Get started free</button>
      </div>
      {/* PRO */}
      <div style={{ border: "1px solid rgba(0,212,255,.3)", borderRadius: 14, padding: "30px 26px", background: "rgba(0,212,255,.03)", position: "relative" }}>
        ...
        <button onClick={() => { if(onSignUp) onSignUp(); document.getElementById("auth")?.scrollIntoView({behavior:"smooth"}); }} className="land-btn-primary" style={{ display: "block", width: "100%", textAlign: "center", marginTop: 24, padding: "12px", border: "none", cursor: "pointer" }}>Start with Pro</button>
      </div>
    </div>
  ...
  <section style={{ textAlign: "center", padding: "80px 24px 120px", borderTop: "1px solid rgba(255,255,255,.04)" }}>
    ...
    <button onClick={() => { if(onSignUp) onSignUp(); document.getElementById("auth")?.scrollIntoView({behavior:"smooth"}); }} className="land-btn-primary land-cta-btn" style={{ fontSize: 16, padding: "18px 44px", border: "none", cursor: "pointer" }}>Get your first brief free</button>
  </section>
