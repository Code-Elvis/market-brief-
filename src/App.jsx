...
{ q: “What is the recommended workflow before and during trading?”, a: “Step 1 — Type your instrument and run a Full Brief. Read the macro theme, high-impact events and geopolitical risks. Step 2 — Run Scalper Mode for a CLEAR / CAUTION / STAND DOWN event awareness check. Step 3 — Check the Breaking Narratives tab every 15 minutes during your session. If a narrative hits that invalidates your setup you see it here first. Paste any headline you don’t understand for an instant macro explanation. Step 4 — If a major event fires (Fed comment, Trump tweet, geopolitical escalation) — run a fresh brief immediately. Adapt your bias. Brief First, Trade After.” },
...
1. EVENTS must be STRICTLY UPCOMING — scheduled in the future from the current time. NEVER include events that have already occurred or already been released today. If an event has already happened, exclude it entirely.
1. For events, only include the 2-3 most market-moving SCHEDULED releases coming up in the next 48 hours that directly affect this instrument. Include the exact scheduled time.
1. Your job is macro context and forward-looking event risk — not technical analysis, not past events.`;
...
const now = new Date().toLocaleString(“en-GB”, { weekday: “long”, year: “numeric”, month: “long”, day: “numeric”, hour: “2-digit”, minute: “2-digit” });
const sys = `You are a professional equity analyst. Respond ONLY with valid JSON. No markdown, no backticks, no preamble. Start with { and end with }. RULES: Never mention specific price levels. Focus on macro, fundamental, and sector context only — current and upcoming only. ` + ‘EQUITY BRIEF schema: {“instrument”:“string”,“ticker”:“string”,“sector”:“string”,“sentiment”:“bullish|bearish|neutral|mixed”,“headline_summary”:“string”,“earnings_context”:“string”,“macro_tailwinds”:“string”,“macro_headwinds”:“string”,“sector_rotation”:“string”,“catalyst_events”:[{“title”:“string”,“time”:“string”,“impact”:“HIGH|MEDIUM”,“direction”:“BULLISH|BEARISH|NEUTRAL”,“summary”:“string”}],“institutional_flow”:“string”,“teaching_moment”:“string”}’;
...
const now = new Date().toLocaleString(“en-GB”, { weekday: “long”, year: “numeric”, month: “long”, day: “numeric”, hour: “2-digit”, minute: “2-digit” });
const sys = `You are a professional equity trader's risk assistant. Respond ONLY with valid JSON. No markdown, no backticks, no preamble. Start with { and end with }. RULES: Never mention specific price levels. Focus only on CURRENT and IMMINENT risks for this specific stock. EQUITY SCALPER schema: {"ticker":"string","risk_level":"GREEN|YELLOW|RED","risk_reason":"string","scalper_note":"string","earnings_proximity":"SAFE|NEAR|IMMINENT","breaking":[{"headline":"string","direction":"BULLISH|BEARISH|NEUTRAL","age":"string"}],"imminent":[{"event":"string","due_in":"string","expected_impact":"string"}]}`;
...
1. NEVER mention specific price levels, targets, stops or support/resistance numbers.
1. Be specific about WHICH macro events or news actually fired in the last session and how the market reacted.
1. The watch_tomorrow field should name ONE specific upcoming event or theme to prepare for in the next trading session.
1. session_summary must be ONE short sentence max — like a trader’s journal entry. Under 120 characters.
1. primary_driver and what_it_revealed must each be ONE sentence, under 100 characters each.
1. ${priceNote}
   POST-SESSION schema: {“instrument”:“string”,“session_summary”:“string”,“primary_driver”:“string”,“secondary_driver”:“string”,“what_it_revealed”:“string”,“watch_tomorrow”:“string”,“next_event”:{“title”:“string”,“time”:“string”}}`;   const msg = `Current time: ${now}. Write a post-session debrief for the most recent TRADING DAY (Mon-Fri) for ${inst.label}. ${priceNote} Cover: (1) how the last trading session played out in one sentence, (2) the primary macro driver, (3) any secondary factor, (4) what it revealed about the macro picture, (5) what to watch in the next trading session, (6) the next scheduled high-impact event. No price levels.`;
   return callClaude(sys, msg);
   }
...
1. NEVER label instruments as BULLISH or BEARISH — instead describe the MECHANISM: what force is acting on this instrument and why.
1. CRITICAL: Always check for cross-instrument tensions. For example if Dollar strengthens, that typically creates headwinds for Gold. If you list both, explicitly note the conflict in the impact text.
1. narrative_summary must be 1-2 punchy sentences — the macro cause-and-effect a trader needs in 10 seconds.
1. Each instrument impact must be ONE sentence describing what macro force is acting on it and why.
1. urgency: CRITICAL (market moving now), HIGH (significant impact expected), MEDIUM (watch closely).
   ...
   tensions field: if any instruments have conflicting forces (e.g. safe haven demand for Gold vs Dollar strength weighing on Gold) — describe the conflict here in one sentence. Leave empty string if no conflicts.`;   const msg = `Current time: ${now}. Breaking headline: “${headline}”. Interpret this for macro traders. For each affected instrument explain the MACRO MECHANISM — what force is acting on it and why. Check for cross-instrument conflicts (e.g. Dollar vs Gold, Risk-off vs Oil). Never use bullish/bearish labels — describe the flow instead.`;
   return callClaude(sys, msg);
   }
